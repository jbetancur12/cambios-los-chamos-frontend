                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant={res.status === 'OK' ? 'default' : 'destructive'} className="text-lg py-1 px-4">
                            {res.status === 'OK' && <span className="flex items-center gap-2"><CheckCircle className="h-4 w-4" /> CONSISTENTE</span>}
                            {res.status === 'INCONSISTENT' && <span className="flex items-center gap-2"><XCircle className="h-4 w-4" /> INCONSISTENTE</span>}
                        </Badge>
                    </div>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-100 dark:border-blue-900">
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between">
                                    <span>Disponible:</span>
                                    <span className="font-mono font-bold text-blue-700">{formatCurrency(res.details.calculatedAvailable)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>A Favor:</span>
                                    <span className="font-mono font-bold text-blue-700">{formatCurrency(res.details.calculatedSurplus)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
                            {res.details.firstTransaction && (
                                <div className="p-4 rounded-lg bg-orange-50 border border-orange-100">
                                    ...
                                    <p className="text-xs text-orange-600/70 mt-1">{new Date(res.details.firstTransaction.date).toLocaleString()}</p>
                                </div>
                            )}
                                ...
                                {res.details.lastTransaction ? (
                                    <div className="space-y-1 text-sm">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-mono font-bold text-purple-700">{formatCurrency(res.details.lastTransaction.storedAvailable)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Deuda Acumulada Calc:</span>
                                            <span className="font-mono font-bold text-red-600">{formatCurrency(res.details.accumulatedDebt)}</span>
                                        </div>
                                    </div>
                                ) : (
                ...
                <CardContent>
                    <div className="bg-black/90 p-4 rounded-lg overflow-x-auto font-mono text-xs md:text-sm text-green-400 max-h-[500px] overflow-y-auto">
                        {res.trace.map((line, i) => (
                            <div key={i} className="whitespace-nowrap pb-1 border-b border-white/5 last:border-0 hover:bg-white/5 px-2">
                                {line}
                            </div>
                        ))}
                    </div>
        ...
        <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
            <div>
                ...
                <p className="text-muted-foreground mt-2">
                    Esta herramienta recalcula el historial completo de transacciones para verificar la integridad matemática de los saldos.
                </p>
            </div>
            ...
            {result && Array.isArray(result) && (
                <div className="grid gap-4">
                    ...
                    {result.filter(r => r.status !== 'OK').map(r => (
                        <div key={r.userId} className="border border-red-200 bg-red-50 p-4 rounded-lg flex justify-between items-center">
                            <div>
                                <p className="font-bold text-red-800">{r.fullName} ({r.email})</p>
                    ...
                    {result.filter(r => r.status === 'OK').map(r => (
                        <div key={r.userId} className="border border-green-200 bg-green-50/50 p-4 rounded-lg flex justify-between items-center opacity-70 hover:opacity-100 transition-opacity">
                            <div>
                                <p className="font-medium text-green-800">{r.fullName}</p>
                                <p className="text-xs text-green-600">{r.email}</p>
                            </div>
                            <Badge variant="outline" className="bg-white text-green-600 border-green-200">OK</Badge>
                        </div>
                    ))}
                </div>
            )}
