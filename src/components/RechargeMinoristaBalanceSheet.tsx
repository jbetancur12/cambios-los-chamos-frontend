...
import { AlertCircle, Eye, DollarSign, Calendar, ChevronDown, Download } from 'lucide-react'
...
export function RechargeMinoristaBalanceSheet({
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const { blob, filename } = await api.downloadFile(url)
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  {surplusAmount > 0
                    ? 'Paga el saldo a favor al minorista (ingresa valor negativo) para reducir el excedente.'
                    : 'Paga la deuda acumulada para liberar cupo disponible.'}
                </p>
              </div>
              ...
              <form onSubmit={handlePayDebt} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="pay-amount">
                    {surplusAmount > 0 ? 'Monto a Pagar (Negativo para descontar)' : 'Monto a Pagar (COP)'}
                  </Label>
                  <div className="flex gap-2">
                    ...
                    <Button
                      type="button"
                      onClick={() =>
                        setPayAmount(surplusAmount > 0 ? (-surplusAmount).toString() : debtAmount.toString())
                      }
                      variant="outline"
